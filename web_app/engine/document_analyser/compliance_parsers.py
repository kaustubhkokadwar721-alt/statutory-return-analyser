...
# NIC "E-WAY BILL SYSTEM" summary — Part-A (parties, value, HSN) + Part-B (vehicle).

def parse_ewb(pdf, fname: str) -> dict:
    text = _page_text(pdf)
    ...
    def gstin_name(label: str):
        m = re.search(rf"{label}\s+(\S+)\s*,\s*(.+)", text, re.IGNORECASE)
        return (m.group(1).strip(), m.group(2).strip()) if m else ("", "")

    irn      = after(r"IRN:", r"(\w+)")
    ack_no   = after(r"Ack\.?\s*No:", r"(\d+)")
    ack_date = after(r"Ack\.?\s*Date:", r"([\d-]+)")
    ewb_no   = re.sub(r"\s+", "", after(r"E-Way Bill No:", r"([\d ]+)"))
    ewb_date = after(r"E-Way Bill Date:", r"([\d/]+)")
    gen_by   = after(r"Generated By:")

    sup_gstin, sup_name = gstin_name(r"GSTIN of Supplier")
    rec_gstin, rec_name = gstin_name(r"GSTIN of Recipient")
    dispatch = after(r"Place of Dispatch")
    delivery = after(r"Place of Delivery")
    doc_no   = after(r"Document No\.?", r"(\S+)")
    doc_date = after(r"Document Date", r"([\d/]+)")
    txn_type = after(r"Transaction Type")
    value    = to_float(after(r"Value of Goods", r"([\d,]+(?:\.\d+)?)"))
    reason   = after(r"Reason For Transportation:")
    transporter = after(r"Transporter")

    # Validity + distance: "Valid From: 16/05/2025 18:52:00[350 Kms]"
    vf = re.search(r"Valid From:\s*([\d/]+)[^\[]*(?:\[(\d+)\s*Kms\])?", text, re.IGNORECASE)
    valid_from = vf.group(1) if vf else ""
    distance   = to_float(vf.group(2)) if vf and vf.group(2) else 0.0
    valid_until = after(r"Valid Until:", r"([\d/]+)")

    # HSN code appears two ways: inline ("HSN Code 84137010-…") or on the line
    # ABOVE a bare "HSN Code" label (with the description below it).
    hsn_code, hsn_desc = "", ""
    inline = re.search(r"HSN Code\s+(\d{6,8})[-\s]([^\n]*)", text, re.IGNORECASE)
    if inline:
        hsn_code, hsn_desc = inline.group(1), inline.group(2).strip()
    else:
        hm = re.search(r"([^\n]*)\nHSN Code\s*\n([^\n]*)", text, re.IGNORECASE)
        if hm:
            code_m = re.search(r"(\d{6,8})", hm.group(1))
            hsn_code = code_m.group(1) if code_m else ""
            hsn_desc = hm.group(2).strip()

    # Part-B vehicle: mode + registration number
    veh_m = re.search(r"\b([A-Z]{2}\d{1,2}[A-Z]{1,3}\d{3,4})\b", text)
    vehicle = veh_m.group(1) if veh_m else ""
    mode_m  = re.search(r"\b(Road|Rail|Air|Ship)\b\s+[A-Z]{2}\d", text)
    mode    = mode_m.group(1) if mode_m else ""

    dd = _parse_date((doc_date or ewb_date).split()[0] if (doc_date or ewb_date) else "")
    if dd is not None and pd.isna(dd):
        dd = None
    fy_str, _ = _fy_from_date(dd)
    period_date = pd.Timestamp(dd.year, dd.month, 1) if dd is not None else None

    return {
        "ReturnType":       "EWB",
        "DocKind":          "Return",
        "EntityID":         sup_gstin or "Unknown",
        "EntityName":       sup_name or "Unknown",
        "FY":               fy_str,
        "EWB_No":           ewb_no,
        "EWB_Date":         _norm_date(ewb_date),
        "IRN":              irn,
        "Ack_No":           ack_no,
        "Ack_Date":         _norm_date(ack_date),
        "Generated_By":     gen_by,
        "Supplier_GSTIN":   sup_gstin,
        "Supplier_Name":    sup_name,
        "Place_of_Dispatch": dispatch,
        "Recipient_GSTIN":  rec_gstin,
        "Recipient_Name":   rec_name,
        "Place_of_Delivery": delivery,
        "Document_No":      doc_no,
        "Document_Date":    _norm_date(doc_date),
        ...
        "Valid_From":       _norm_date(valid_from),
        "Valid_Until":      _norm_date(valid_until),
        "Distance_Km":      distance,
        "Vehicle_No":       vehicle,
        "Mode":             mode,
        "DocRef":           ewb_no,
        "FilingDate":       _norm_date(ewb_date),
        "PeriodDate":       period_date,
        "Month":            period_date.strftime("%B") if period_date is not None else "Unknown",
    }
